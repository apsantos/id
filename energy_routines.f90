...
CONTAINS
        ...
        !------------------------------
!FSL adicionar subrotina Charge_Factor_EPS_r, que define charge factor como f(T)
  SUBROUTINE Charge_Factor_EPS_r(box_t,stp)
    !Dada a molecula e sua especie, computa temperatura da caixa e entao o charge_factor
...
!    INTEGER :: box_t
    ...
    IF(imp_wat) THEN
       IF(fix_stp) THEN
          stp = stp_val
       ELSE
       tempc = temperature(box_t) - tempK
       stp = T0 + T1*tempc + T2*tempc*tempc + T3*tempc*tempc*tempc
       ENDIF
    ELSE
       stp = 1
    ENDIF
  END SUBROUTINE Charge_Factor_EPS_r
  !----------------------------------------------------------------------------------------------
...
!AV: AMBER dihedral style
  ...
  SUBROUTINE Compute_Atom_Nonbond_Energy(this_atom,this_molecule,this_species,E_intra_vdw, &
    ...
    REAL(DP) :: qi, qj, rij, erf_val, erfc_val, qsc, stp
    REAL(DP) :: T, x, xsq, TP
...
!FSL Call charge_factor
    CALL Charge_Factor_EPS_r(this_box,stp)
             ...
             DO ia=1,natoms(is)
                ...
                IF(.NOT. overlap .AND. rijsq <= rcutsq) THEN         
                   ...
                   ELSE
                      ...
                      Eij_qq = (qi*qj/rij)*(qsc - erf_val)*charge_factor/stp
...
!    !$ include 'omp_lib.h'
  ...
  SUBROUTINE Pair_Energy &
    ...
    !FSL: adicionamos a energia de hidratacao aqui, que sera computada apenas para os atomos certos
...
!FSL inicio de definicoes para E_hyd
    REAL(DP) :: E_hyd, prehyd, exphyd, rhij
!FSL fim das definicoes para E_hyd e inicio de definicoes de Eqqcor
    Real(DP) :: qi,qj, qsc, stp
    REAL(DP) :: this_lambda, RsqOverSig, R6OverSig, factorLJ
    REAL(DP) :: RsqOverSig_Shift, RsqOverSig6_Shift, factorLJ_Shift
!FSL inicio de definicoes para interacoes WCA
    REAL(DP) :: rwca
!FSL fim das definicoes para interacoes WCA    
...
!FSL Call charge_factor
    CALL Charge_Factor_EPS_r(ibox,stp)

!FSL adicionando a E_hyd aqui. nao conferimos se estao na mesma caixa pois isso
!e feito em compute_molecule-nonbond_inter_energy

!mol 1 = 2 = cation
         IF(.NOT. imp_wat) THEN
          E_hyd = 0.00_DP
         ELSE
             IF (is == 1 .AND. js == 1 .AND. ia == ah1 .AND. &
                   ja == ah1 .AND. im /= jm) THEN
                rhij = DSQRT(rijsq)
                prehyd = Hh1 / (sh1 * DSQRT(twoPI))
                exphyd = EXP(-((rhij - rh1)**2)/(2 * sh1**2))
                E_hyd = prehyd * exphyd
!mol 1 /= mol 2
             ELSEIF (is == 1 .AND. js == 2 .AND. ia == ah1 .AND. ja == ah2 .OR. &
                     is == 2 .AND. js == 1 .AND. ia == ah2 .AND. ja == ah1) THEN
               rhij  =DSQRT(rijsq)
               prehyd = Hh2 / (sh2 * DSQRT(twoPI))
               exphyd = EXP(-((rhij - rh2)**2)/(2 * sh2**2))
               E_hyd = prehyd * exphyd
!mol 1 = 2 = anion
             ELSEIF (is == 2 .AND. js == 2 .AND. ia == ah2 .AND. &
                       ja == ah2 .AND. im /= jm) THEN
              rhij = DSQRT(rijsq)
              prehyd = Hh3 / (sh3 * DSQRT(twoPI))
              exphyd = EXP(-((rhij - rh3)**2)/(2*sh3**2))
              E_hyd  = prehyd * exphyd
             ELSE
                E_hyd = 0.0_DP
             ENDIF
    ...
    ExistCheck: IF (atom_list(ia,im,is)%exist .AND. atom_list(ja,jm,js)%exist) THEN
       ...
       VDW_calculation: IF (get_vdw) THEN
          ...
          LJ_12_6_calculation: IF (int_vdw_style(1) == vdw_lj) THEN
             ...
             IF (is == 1 .AND. js == 2 .AND. ia == ah1 .AND. ja == ah2 .OR. &
                 is == 2 .AND. js == 1 .AND. ia == ah2 .AND. ja == ah1) THEN
                     eps = eps*epsscale
                     sig = sig*sigscale
             ENDIF

!             IF (is == 1 .AND. js == 1 .AND. im /= jm .AND. &
!                ANY(tail == ia) .AND. ANY(tail == ja)) then
!                eps = eps*hescale
!             ENDIF
             ...
             SigOverR12 = SigOverR6 * SigOverR6
!FSL calculando rwca
             rwca = 1.26_DP*sig*sig
!FSL final de calculo de rwca
...
!FSL potencial WCA             
!           IF(surfsim) THEN
!             IF (is == 1 .AND. js == 1 .AND. ia == 13 .AND. &
!              ja /= 13 .AND. im /= jm .OR. is == 1 .AND. &
!              js == 1 .AND. ia /= 13 .AND. &
!              ja == 13 .AND. im /= jm) THEN
!              IF (rijsq .LE. rwca) THEN
!                 Eij_vdw = Eij_vdw + erep
!              ELSE
!                 Eij_vdw = 0.0_DP
!              ENDIF
!             ENDIF
!           ENDIF
           IF(surfsim) THEN
             IF (is == 1 .AND. js == 1 .AND. ANY(head .EQ. ia) .AND. &
              ANY(head .NE. ja) .AND. im /= jm .OR. is == 1 .AND. &
              js == 1 .AND. ANY(head .NE. ia) .AND. &
              ANY(head .EQ. ja) .AND. im /= jm) THEN
              IF (rijsq .LE. rwca) THEN
                 Eij_vdw = Eij_vdw + erep
              ELSE
                 Eij_vdw = 0.0_DP
           ...
           ENDIF
           IF(surfsim) THEN
             IF (is == 1 .AND. js == 2 .AND. ANY(tail .EQ. ia) &
              .OR. is == 2 .AND. js == 1 .AND. ANY(tail .EQ. ja)) THEN
              IF (rijsq .LE. rwca) THEN
                 Eij_vdw = Eij_vdw + erep
              ELSE
                 Eij_vdw = 0.0_DP
           ...
           ENDIF
!           IF(surfsim) THEN
!             IF (is == 1 .AND. js == 2 .AND. ia /= 13 &
!              .OR. is == 2 .AND. js == 1 .AND. ja /= 13) THEN
!              IF (rijsq .LE. rwca) THEN
!                 Eij_vdw = Eij_vdw + erep
!              ELSE
!                 Eij_vdw = 0.0_DP
...
!FSL final potencial WCA             
       ...
       qq_calculation: IF (get_qq) THEN
          ...
          IF (int_charge_sum_style(ibox) == charge_cut .OR. igas_flag) THEN
             ...
             END IF
             Eij_qq = qsc*charge_factor*(qi*qj)/(SQRT(rijsq)*stp)
  ...
  !------------------------------------------------------------------------------------------
    ...
    REAL(DP) :: qi,qj,qsc,rijsq,rij,erf_val,stp
    REAL(DP) :: Eij

!FSL fim das definicoes para E_hyd e inicio de definicoes de Eqqcor
    REAL(DP) :: Eqqcor, rqqcor, edr
...
!FSL Call charge_factor
    ...
    Eij = (qi*qj/rij)*(qsc - erf_val)*charge_factor/stp
!                   IF(en_flag) THEN
!                      WRITE(60,"(4I4,2F8.5,F24.12)") ia, ja, jm, js, qi,qj, Eij
!                   END IF
!FSL adicionada energia de interacao Eqqcor para is = 1 and ia = N e js = 2
!usando o COM de jm
       IF(.NOT. imp_wat) THEN
        Eqqcor = 0.00_DP
       ELSE
!mol 1 = 2 = cation
             IF (is == 1 .AND. js == 1 .AND. ia == ah1 .AND. &
                   ja == ah1 .AND. im /= jm) THEN
              rqqcor = DSQRT(rijsq)
              edr = (5.2_DP + stp)/2 + ((stp - 5.2_DP)/2)*tanh((rqqcor-rme1)/sh1)
              Eqqcor = charge_factor*(stp/edr - 1)/(rqqcor*stp)
!mol 1 /= mol 2
             ELSEIF (is == 1 .AND. js == 2 .AND. ia == ah1 .AND. ja == ah2 .OR. &
                     is == 2 .AND. js == 1 .AND. ia == ah2 .AND. ja == ah1) THEN   
              rqqcor = DSQRT(rijsq)
              edr = (5.2_DP + stp)/2 + ((stp - 5.2_DP)/2)*tanh((rqqcor-rme2)/sh2)
              Eqqcor = -charge_factor*((stp/edr) - 1)/(rqqcor*stp)
!mol 1 = 2 = anion
             ELSEIF (is == 2 .AND. js == 2 .AND. ia == ah2 .AND. &
                       ja == ah2 .AND. im /= jm) THEN
              rqqcor=DSQRT(rijsq)
              edr = (5.2_DP + stp)/2 + ((stp - 5.2_DP)/2)*tanh((rqqcor-rme3)/sh3)
              Eqqcor = charge_factor*(stp/edr - 1)/(rqqcor*stp)
          ELSE
                  Eqqcor = 0.00_DP
          ENDIF
       ENDIF
!FSL terminado de adicionar a energia de coulomb corrigida
...
!    !$ include 'omp_lib.h'
    ...
    REAL(DP) :: this_scaling, cos_sum_locate, sin_sum_locate, stp
...
!FSL Call charge_factor
    ...
    energy(this_box)%ewald_reciprocal=energy(this_box)%ewald_reciprocal*charge_factor/stp
...
!FSL Call charge_factor
    ...
    IF ( move_flag == int_translation .OR. move_flag == int_rotation .OR. move_flag == int_intra ) THEN
       ...
       v_recip_difference = v_recip_difference * charge_factor/stp - energy(this_box)%ewald_reciprocal
    ...
    ELSE IF ( move_flag == int_deletion) THEN
       ...
       END DO
       !$OMP END PARALLEL DO
       v_recip_difference = v_recip_difference * charge_factor/stp
...
!FSL Call charge_factor
    ...
    energy(this_box)%ewald_self=-energy(this_box)%ewald_self*charge_factor/stp
...
!FSL Call charge_factor
    ...
    IF ( move_flag == int_insertion ) THEN
       ...
       V_self_difference=-charge_factor*V_self_difference*alpha_ewald(this_box)/(rootPI*stp)
...
!        IF(surfsim) THEN
!         IF (is == 1 .AND. js == 1 .AND. ANY(head .EQ. ia) .AND. &
!          ANY(head .NE. ja) .OR. is == 1 .AND. js == 1 .AND. &
!          ANY(head .NE. ia) .AND. ANY(head .EQ. ja)) THEN
!               CYCLE
!         ELSE
!
...
!******************************************************************************************
 ...
 SUBROUTINE Compute_Forces(this_box)
   ...
   REAL(DP) :: rcom, rx, ry, rz, w_lrc, stp
...
!                W_tensor_charge(:,:,this_box) = W_tensor_charge(:,:,this_box) + tc_pair(:,:)
    ...
    W_tensor_elec(:,:,this_box)=(W_tensor_charge(:,:,this_box)+W_tensor_recip(:,:,this_box))*charge_factor/stp
  ...
  !------------------------------------------------------------------------------------------
    ...
    REAL(DP) :: SigOverRsq_shift,SigOverR6_shift,SigOverR12_shift
    REAL(DP) :: roffsq_rijsq, roffsq_rijsq_sq, factor2, fscale
    REAL(DP) :: qi,qj, qsc, erf_val, this_lambda, stp
       ...
       qq_calculation: IF (get_qq) THEN
          ...
          IF (int_charge_sum_style(ibox) == charge_cut) THEN
             ...
             IF ( is == js .AND. im == jm ) THEN
                qsc = charge_intra_scale(ia,ja,is)
             END IF
             Wij_qq = qsc*charge_factor*(qi*qj)/(SQRT(rijsq)*stp)
...
!    !$ include 'omp_lib.h'
    ...
    REAL(DP) :: charge, hdotr, energy_temp, stp
...
!FSL Call charge_factor
    ...
    energy(this_box)%ewald_reciprocal = energy_temp * charge_factor/stp
