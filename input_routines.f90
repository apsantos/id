...
!********************************************************************************
  ...
  DO
    ...
    ELSE IF (line_string(1:13) == '# Pair_Energy') THEN
        ...
        END IF
        WRITE(logunit,*)
!FSL ver se sim e com solvente implicito ou nao
    ELSEIF (line_string(1:16) == '# Implicit_Water') THEN
            line_nbr = line_nbr + 1
            ...
            IF (line_array(1) == 'TRUE' .OR. line_array(1) == 'true') THEN
                    imp_wat = .TRUE.
            END IF
            IF (line_array(2) == 'TRUE' .OR. line_array(2) == 'true') THEN
                    fix_stp = .TRUE.
            END IF
            stp_val = String_To_Double(line_array(3))
!FSL fim de agua implicita
!FSL inicio parametros de E_hyd
     ELSEIF (line_string(1:18) == '# Hydration_Energy') THEN
        line_nbr = line_nbr + 1
        CALL Parse_String(inputunit,line_nbr,2,nbr_entries,line_array,ierr)
        ah1 = String_To_Double(line_array(1))
        ah2 = String_To_Double(line_array(2))
        line_nbr = line_nbr + 1
        CALL Parse_String(inputunit,line_nbr,3,nbr_entries,line_array,ierr)
        Hh1 = String_To_Double(line_array(1))*100
        Hh2 = String_To_Double(line_array(2))*100
        Hh3 = String_To_Double(line_array(3))*100
        line_nbr = line_nbr + 1
        CALL Parse_String(inputunit,line_nbr,3,nbr_entries,line_array,ierr)
        sh1 = String_To_Double(line_array(1))
        sh2 = String_To_Double(line_array(2))
        sh3 = String_To_Double(line_array(3))
        line_nbr = line_nbr + 1
        CALL Parse_String(inputunit,line_nbr,3,nbr_entries,line_array,ierr)
        rh1 = String_To_Double(line_array(1))
        rh2 = String_To_Double(line_array(2))
        rh3 = String_To_Double(line_array(3))
!FSL fim de parametros de E_hyd
!FSL inicio de parametros de Eqqcor
        ELSEIF(line_string(1:20) == '# Coulomb_Correction') THEN
                line_nbr = line_nbr + 1
                CALL Parse_String(inputunit,line_nbr,3,nbr_entries,line_array,ierr)
                rme1 = String_To_Double(line_array(1))
                rme2 = String_To_Double(line_array(2))
                rme3 = String_To_Double(line_array(3))
!FSL fim de parametros de Eqqcor - sigma ja foi definido em E_hyd acima
!FSL definicao de escalas para epsilon e sigma
        ELSEIF(line_String(1:10) == '# LJ_Scale') THEN
           line_nbr = line_nbr + 1
           CALL Parse_String(inputunit,line_nbr,2,nbr_entries,line_array,ierr)
           epsscale = String_To_Double(line_array(1))
           sigscale = String_To_Double(line_array(2))
!FSL fim de escala para LJ
!FSL peso para pegar surfactante ou contra-ion
        ELSEIF(line_String(1:21) == '# Ins_Del_Weight_Surf') THEN
           line_nbr = line_nbr + 1
           CALL Parse_String(inputunit,line_nbr,1,nbr_entries,line_array,ierr)
           idw_surf = String_To_Double(line_array(1))
!FSL fim de peso para pegar surfactante ou contra-ion           
!FSL definicao de atomos da cauda ou cabeca polar
        ELSEIF(line_String(1:11) == '# Head_Tail') THEN
           line_nbr = line_nbr + 1
           CALL Parse_String(inputunit,line_nbr,3,nbr_entries,line_array,ierr)

           IF (line_array(1) == 'TRUE' .OR. line_array(1) == 'true') THEN
              surfsim = .TRUE.
              erep = String_To_Double(line_array(2))*100
              hescale = String_To_Double(line_array(3))
           ENDIF
!FSL fim de definicao de atomos da cauda ou cabeca polar
     ELSEIF (line_string(1:3) == 'END' .or. line_nbr > 10000) THEN
        IF (iassign == 2*nbr_boxes) EXIT
...
!********************************************************************************
 ...
 DO
     ...
     IF (ierr /= 0) THEN
        err_msg = ""
        err_msg(1) = "Error reading intrascaling info."
